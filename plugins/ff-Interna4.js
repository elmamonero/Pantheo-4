// Memoria global para almacenar las listas activas de interna4
global.interna4 = global.interna4 || {};

const handler = async (m, { conn, args }) => {
    if (args.length < 3) {
        conn.reply(m.chat, 'Debes proporcionar la hora (HH:MM), AM/PM y el país (MX, CO, CL, AR, PE, EC).\nEjemplo: *.interna4 08:00 PM CO*', m);
        return;
    }
    ...
    // Calcular la fecha local según la zona horaria dada
    const now = new Date();
    ...
    global.interna4[sentMsg.key.id] = {
        chat: m.chat,
        horasEnPais,
...
handler.before = async function (m, { conn }) {
    ...
    if (!global.interna4 || !global.interna4[msgId]) return;

    const game = global.interna4[msgId];
    const tiempoLimite = 20 * 60 * 1000;
    if (Date.now() - game.timestamp > tiempoLimite) {
        delete global.interna4[msgId];
        return;
    }

    // Limpiar participante previo en otras posiciones
...
│
│ㅤʚ 𝗝𝗨𝗚𝗔𝗗𝗢𝗥𝗘𝗦: (${e1.length + e2.length}/8)
│
│    𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 1 (${e1.length}/4)
│👑 ➤ ${slotE1(0)}
│⚜️ ➤ ${slotE1(1)}
│⚜️ ➤ ${slotE1(2)}
│⚜️ ➤ ${slotE1(3)}
│
│    𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 2 (${e2.length}/4)
│👑 ➤ ${slotE2(0)}
│⚜️ ➤ ${slotE2(1)}
│⚜️ ➤ ${slotE2(2)}
│⚜️ ➤ ${slotE2(3)}
