// Memoria global para almacenar los 4vs4fem activos
global.versus4v4fem = global.versus4v4fem || {};

const handler = async (m, { conn, args }) => {
    if (args.length < 3) {
        conn.reply(m.chat, 'Debes proporcionar la hora (HH:MM), AM/PM, país (MX, CO, CL, AR, PE, EC) y opcionalmente la casilla.\nEjemplo: *.4vs4fem 08:00 PM MX C4*', m);
        return;
    }
    ...
    global.versus4v4fem[sentMsg.key.id] = {
        chat: m.chat,
        horasEnPais,
        casilla,
...
handler.before = async function (m, { conn }) {
    ...
    if (!global.versus4v4fem || !global.versus4v4fem[msgId]) return;

    const game = global.versus4v4fem[msgId];
    ...
    if (tiempoTranscurrido > diecisieteMinutos) {
        delete global.versus4v4fem[msgId];
        return;
    }
