...
let handler = async(m, { conn, text, usedPrefix, command }) => {
  ...
  try {
    ...
    // Validación del formato de respuesta de Yuki (revisa si viene el array en 'data')
    if (!json || json.status !== true || !json.data || !Array.isArray(json.data) || json.data.length === 0) {
      return m.reply('*[ ❌ ] No se encontró la letra de esta canción.*');
    }

    // ARREGLADO: Tomamos el primer resultado del array devuelto en json.data
    let crow = json.data[0];

    // Extraemos la letra de forma segura
    let lyrics = crow?.lyrics || crow?.letra;

    if (!lyrics) {
      return m.reply('*[ ❌ ] La canción fue encontrada pero no tiene una letra disponible.*');
    }

    // Buscamos la imagen de la canción en YouTube para el mensaje del Bot
    let res = await yts(text);
    let thumb = res.videos[0]?.thumbnail || 'https://telegra.ph/file/243e111fef951ecc20f4c.png';

    // Formateamos las propiedades o tomamos alternativas por si acaso
    let title = crow?.title || res.videos[0]?.title || text;
    let artist = crow?.artist || crow?.author || res.videos[0]?.author?.name || 'Desconocido';
    let album = crow?.album || 'Single';
    
    // Si la duración viene en segundos la convertimos a MM:SS, sino usamos la de YT
    let duration = 'Desconocida';
    if (crow?.duration) {
      let mins = Math.floor(crow.duration / 60);
      let secs = Math.floor(crow.duration % 60);
      duration = `${mins}:${secs.toString().padStart(2, '0')}`;
    } else {
      duration = res.videos[0]?.timestamp || 'Desconocida';
    }
