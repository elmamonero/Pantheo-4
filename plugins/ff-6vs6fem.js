// Memoria global para almacenar los 6vs6fem activos
global.versus6v6fem = global.versus6v6fem || {};

const handler = async (m, { conn, args }) => {
    if (args.length < 3) {
        conn.reply(m.chat, 'Debes proporcionar la hora (HH:MM), AM/PM, país (MX, CO, CL, AR, PE, EC) y opcionalmente la casilla.\nEjemplo: *.6vs6fem 08:00 PM MX C4*', m);
        return;
    }
    ...
    global.versus6v6fem[sentMsg.key.id] = {
        chat: m.chat,
        horasEnPais,
        casilla,
...
handler.before = async function (m, { conn }) {
    ...
    if (!global.versus6v6fem || !global.versus6v6fem[msgId]) return;

    const game = global.versus6v6fem[msgId];
    ...
    if (tiempoTranscurrido > diecisieteMinutos) {
        delete global.versus6v6fem[msgId];
        return;
    }
